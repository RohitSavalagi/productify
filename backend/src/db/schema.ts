...
// `one()` means a single related record, `many()` means multiple records

export const productsRelations = relations(products, ({ one, many }) => ({
    comments: many(comments),
    // `fields` = the foreign key column in THIS table (products.userId)
    // `references` = the primary key column in the RELATED table (user.id)
    user: one(users, { fields: [products.userId], references: [users.id] }), // on product -> one user
}));
